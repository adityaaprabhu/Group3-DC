...
- Forwards request to SERVER 2 (replica — may have delayed/outdated files)
- Compares files and sends result to CLIENT
  * If files match   → sends one copy to CLIENT
  * If files differ  → sends BOTH (replication delay detected)
  * If only one has  → sends that file
  * If none found    → sends not found message
"""
    ...
    try:
        ...
        if s1_found and s2_found:
            if s1_content == s2_content:
                ...
                print("[SERVER1] Files DIFFER (possible replication delay). Sending both files to CLIENT.")
                response = {
                    'status': 'mismatch',
                    'message': 'Files differ between servers — possible replication delay! Sending both versions.',
